...
{
    private ResourceManager _resources = new ResourceManager("ComputerLock.resource.lang.FmLockScreen", typeof(FmLockScreen).Assembly);
    public FmLockScreen()
    {
        InitializeComponent();
    }
    ...
    public void Open()
    {
        ...
        LblPassword.Text = _resources.GetString("LblPassword", CultureInfo.CurrentCulture);
        _isLocked = true;
        TxtPassword.Text = "";
        LblMessage.Text = $"{_resources.GetString("LblTimerPrefix", CultureInfo.CurrentCulture)}{_hideSelfSecond}{_resources.GetString("LblTimerPostfix", CultureInfo.CurrentCulture)}";
    ...
    private void timer1_Tick(object sender, EventArgs e)
    {
        try
        {
            ...
            if (AppBase.Config.IsHidePasswordWindow)
            {
                ...
                LblMessage.Text = $"{_resources.GetString("LblTimerPrefix", CultureInfo.CurrentCulture)}{hideCountdown}{_resources.GetString("LblTimerPostfix", CultureInfo.CurrentCulture)}";
                if (hideCountdown <= 0)
                {
                    HideSelf();
                }
